...
//   const [faqState, setFaqState] = useState(singleProduct?.data?.selectMaterialItems?.faqAdd);
...
//           <img
//             src={`${url}${singleProduct?.data?.selectMaterialItems?.images[0]?.image}`}
//             alt="Product"
...
function ProductOtherInfo() {
  ...
  const [updateProductFaqAndSizeMaterial] = useUpdateProductFaqSizeMaterialMutation();
  ...
  useEffect(() => {
    if (singleProduct?.data?.selectMaterialItems && singleProduct.data.selectMaterialItems.length > 0) {
      ...
      setFormData({
        id: singleProduct.data.selectMaterialItems[0]?.sizeMaterialAdd?.id || '',
        chain: singleProduct.data.selectMaterialItems[0]?.sizeMaterialAdd?.chain || '',
        styleCollection: singleProduct.data.selectMaterialItems[0]?.sizeMaterialAdd?.styleCollection || '',
        pendantHeight: singleProduct.data.selectMaterialItems[0]?.sizeMaterialAdd?.pendantHeight || '',
        mainMaterial: singleProduct.data.selectMaterialItems[0]?.sizeMaterialAdd?.mainMaterial || '',
        chainLength: singleProduct.data.selectMaterialItems[0]?.sizeMaterialAdd?.chainLength || '',
        pendantThickness: singleProduct.data.selectMaterialItems[0]?.sizeMaterialAdd?.pendantThickness || '',
      });
    }
  }, [singleProduct]);
    ...
    <div className="min-h-[90vh]">
      ...
      <div className="mt-5">
        ...
        <form onSubmit={handleSubmitFaq}>
          <label className="text-base font-semibold inline-block mb-1">Question</label>
          <input
            type="text"
            name="question"
            value={question}
            ...
            className="border py-2 px-2 w-full mb-2"
          />
          <label className="text-base font-semibold inline-block mb-1">Answer</label>
          <input
            type="text"
            name="answer"
            value={answer}
            ...
            className="border py-2 px-2 w-full mb-2"
          />
          <button type="submit" className="bg-orange-300 p-2 text-base font-semibold px-5 mt-2">
            Add FAQ
          </button>
        </form>
        ...
        {faqState?.map((faq, index) => (
          <div key={index} className="flex justify-between items-center mb-2 p-2 border rounded">
            <div>
              <h2><span className="font-semibold">Question:</span> {faq.question}</h2>
